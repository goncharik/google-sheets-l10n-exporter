...
function exportSheet(e) {
    ...
    if (options.language === LANGUAGE_ANDROID) {
        var androidXml = makeAndroidString(processedData.regular, processedData.plurals, 0, options);
        outputStrings.push({ androidXml: androidXml });
    } else if (options.language === LANGUAGE_IOS) {
        if (options.modern) {
            // New .xcstrings format (combines both regular strings and plurals)
            var xcstrings = makeIosXCStrings(processedData.regular, processedData.plurals, options);
            outputStrings.push({ xcstrings: xcstrings });
        } else {
            // Legacy format (not used in this version)
            var iosStrings = makeIosString(processedData.regular, 0, options);
            var output = { iosStrings: iosStrings };
            
            if (Object.keys(processedData.plurals).length > 0) {
                output.iosStringsdict = makeIosStringsdict(processedData.plurals, 0, options);
            }
...
function displayTexts_(languageOutputs, options) {
    ...
    for (var i = 0; i < languageOutputs.length; i++) {
        ...
        if (options.language === LANGUAGE_ANDROID) {
            ...
            if (options.modern) {
                htmlContent += makeTextBox("export_ios_xcstrings_" + i, langOutput.xcstrings, "Localizable.xcstrings");
            } else {
    ...
    SpreadsheetApp.getUi().showModalDialog(app, "Translations (" + options.language + (options.modern ? " Modern" : "") + ")");
...
/*
   ...
   (Kept for reference, not used in the modern version)
*/
